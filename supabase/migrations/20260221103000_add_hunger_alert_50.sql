...
alter table public.pet_state
  add column if not exists hunger_alert_50_sent_at timestamptz,
  add column if not exists hunger_alert_50_message_id uuid references public.messages(id) on delete set null,
  add column if not exists hunger_alert_50_triggered_by uuid references auth.users(id) on delete set null;
