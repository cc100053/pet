...
-- Bug fix: passive hunger decay (tick_pet_state) only mutated the main pet's
-- pet_state, never room_pet_state. Since room_pet_state is the canonical
-- shared state that set_room_main_pet/sync_room_pet_state_to_main_pet_state
-- read from, the two diverged over time. Switching the main pet then copied
-- the stale (un-decayed) room_pet_state hunger onto the newly promoted pet,
-- making it appear to suddenly starve/depart. We now mirror the decay result
-- back into room_pet_state so the shared state and the main pet's mirror stay
-- in lockstep.
...
begin
  ...
  -- Mirror the decay into the canonical shared room state so switching the
  -- main pet does not resurrect stale hunger.
