-- Remove the 10-minute feed burst gate so every feed adds +25 hunger.
--
-- Redefines `apply_pet_action` and `apply_room_pet_action` from `20260808150000`
-- with only the `v_feed_burst_count >= 1 -> v_overfed` check removed. `v_overfed`
-- now stays false, so `last_overfed_at` is no longer stamped and
-- `feed_validate` reports `overfed: false`. Signatures, return types, grants,
-- and the feed coin reward cooldown (`claim_action_reward`) are unchanged.
