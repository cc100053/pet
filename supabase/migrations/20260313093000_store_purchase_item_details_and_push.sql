...
drop function if exists public.purchase_room_background_with_coins(uuid, uuid);
drop function if exists public.purchase_room_background_with_diamonds(uuid, uuid);
...
returns table (remaining_coins int, new_quantity int, message_id uuid)
language plpgsql
security definer
set search_path = public
as $$
...
begin
  ...
  return query select (v_current_coins - v_price), coalesce(v_new_quantity, 0), v_message_id;
end;
$$;
...
returns table (remaining_diamonds int, new_quantity int, message_id uuid)
language plpgsql
security definer
set search_path = public
as $$
...
begin
  ...
  return query select (v_current_diamonds - v_price), coalesce(v_new_quantity, 0), v_message_id;
end;
$$;
