-- Fix: feeding a pet then switching the main pet reverted hunger to the
-- pre-feed value.
--
-- Root cause: the client feed path calls apply_pet_action, which only writes
-- the main pet's pet_state. Hunger/mood are room-shared in room_pet_state, and
-- set_room_main_pet -> sync_room_pet_state_to_main_pet_state copies room_pet_state
-- onto the promoted pet. Because the feed never touched room_pet_state, the swap
-- restored the stale (pre-feed) hunger.
--
-- Fix: mirror the action result into room_pet_state, mirroring the dual-write
-- that tick_pet_state already does, so the shared state stays authoritative.
