-- Fix poop spawn bug: reset last_poop_spawn_at when cleaning, and reduce interval to 8 hours.
-- Bug: When cleaning, last_poop_spawn_at was not reset, so next poop waits from old timestamp.
-- Change: Reset last_poop_spawn_at to current time on clean, reduce interval from 12h to 8h.

-- Recreate tick_pet_state with 8-hour interval
...
declare
  ...
  v_poop_interval interval := interval '8 hours';  -- Changed from 12 to 8 hours
  v_max_poop int := 3;
  v_spawn_x numeric;
  v_spawn_y numeric;
begin
...
-- Recreate apply_pet_action with bug fix: reset last_poop_spawn_at on clean
...
begin
  ...
  if p_action_type = 'feed' then
    ...
    v_poop_count := 0;
    v_poop_positions := '[]'::jsonb;
    v_last_poop_spawn_at := v_now;  -- BUG FIX: Reset last_poop_spawn_at to now!
...
-- Also update clean_poop to reset last_poop_spawn_at when all poop is cleaned
...
begin
  ...
  set poop_positions = v_new_positions,
      poop_count = v_poop_count,
      poop_at = v_poop_at,
      last_poop_spawn_at = now()  -- BUG FIX: Reset when all poop is cleaned
  where pet_id = p_pet_id;
