...
- 等級系統：餵食獲得獎勵時 +10 EXP；升級所需 EXP = 50 * 當前等級；最高等級 999；EXP/糖果獎勵共用冷卻 (每動作每房間 1 小時)。
- 命名：雙方隨時可更改寵物名字。
...
- 餵食增益：每次成功餵食 +20 飽食度（單次套用）。
- 過飽機制：10 分鐘內僅允許 1 次有效餵食；重複餵食觸發對話「好飽呀～」，不扣心情但不加數值。

#### B. 心情 (Mood)
- 等級：Mid -> High -> Sad (悲傷/生病)。✅
- 提升方式：摸頭 (Touch) / 餵食 (Feed) / 清理大便 (Clean poop)。✅
- 每個動作提升 1 個等級。✅
...
### 4.1 貨幣 (Candy / Diamond)
- 獲取途徑：
  - 糖果 (Candy)：透過餵食/清理/摸頭獲得。✅
    - 餵食：+10 Candy。
    - 清理大便：+5 Candy。
    - 摸頭：+1 Candy。
  - 鑽石 (Diamond)：透過現金購買取得，用於購買高級商品或兌換糖果。
- 獲取限制：每種動作 每小時只能獲得一次 糖果獎勵 (防止刷錢)。✅
- 每日任務獎勵：完成 Daily Quest (如：影雪糕) -> 獲得 雙倍糖果 (+20)。
...
- 判定邏輯：
  - 任務匹配 (Match)：如任務是 "Coffee"，AI 識別出 "Drink/Cup" -> 成功 (+20 Candy, Mood Up)。
  - 非任務 (Normal)：AI 識別出任何物件 -> 成功 (+10 Candy, Mood Up)。
  - 識別失敗 (Unknown/Fail)：AI 信心分數過低 -> 當作普通分享 (無 Candy, 無 Mood 變化, 但照片仍發送至聊天室)。
- 冷卻檢查：若 1 小時內已獲獎勵，則僅發送照片，不加糖果。
...
聊天室是遊戲的「客廳」，整合以下三種訊息：
- 文字對話：用戶輸入的 Chat。
- 餵食記錄：帶有特殊邊框的照片卡片 (顯示：[圖片] +10 Candy)。
- 系統事件：灰色小字 (如：「B君 幫忙清理了大便」)。
  - 清理大便：顯示「誰清理」以及「+5 Candy」(例：清理大便：+5 Candy)。
...
- 元件庫：聊天氣泡、照片卡片 (含 +Candy)、系統事件卡、狀態數值提示、Grab Handle、相機按鈕。
...
- 插屏廣告 (Interstitial)：改為可選的「雙倍獎勵」激勵視頻流程：
  - 餵食完成 -> 獲得 10 Candy -> 彈窗「看廣告獲得雙倍 (20 Candy)？」-> 用戶主動點擊播放。
- Banner 廣告：僅在「設定」或「商店」頁面底部，不干擾主遊戲區。
- 激勵視頻 (Rewarded)：主動觀看以獲得糖果或縮短冷卻時間。

### 8.2 內購 (IAP)
- 訂閱 (Subscription)：去除所有廣告 + 無限房間。
- 消耗品 (Consumable)：僅保留鑽石包（用於購買其他道具）。
- IAP 僅兩個品項：月訂閱 + 鑽石包。

### 8.3 商店貨幣規則
- 商店內所有非 IAP 物品皆可用「糖果或鑽石」購買。
- 背景/家具等外觀物品同樣支援雙幣別。
