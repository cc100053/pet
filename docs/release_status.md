...
| --- | --- | --- | --- | --- | --- |
| 2026-08-02 | Supabase Edge Function | `notify_friend` stale-token pruning fix on `ilxzpszgirhwxpeocygs` | **CODE MERGED, NOT DEPLOYED — live is still v31 with the broken matcher** | Behavior-preserving except that dead tokens now actually get pruned. `isStaleTokenFailure` never matched the live FCM HTTP v1 response: FCM pretty-prints `"errorCode": "UNREGISTERED"` (space after the colon) while the matcher looked for the compact `"errorCode":"UNREGISTERED"` plus three legacy/Admin-SDK strings, so the existing auto-delete at `index.ts` had never fired on the current API. Now reads the structured `error.details[].errorCode`, with a whitespace-stripped substring fallback for older/proxied shapes. `INVALID_ARGUMENT` is deliberately still NOT treated as stale, because a malformed message also returns 400 and trusting it would let one bad payload delete every recipient's registration. No request/response contract change; `verify_jwt` must stay `false` for webhook compatibility | Verified the old matcher returns `false` and the new one returns `true` against the two byte-exact 404 bodies taken from `notification_delivery_logs`, plus negative cases for 400 INVALID_ARGUMENT, 500 INTERNAL, and a 502 HTML body. `deno check` clean; `flutter analyze` clean; `flutter test` 558 passed/1 skipped incl. `test/notify_friend_stale_token_pruning_test.dart`. Measured impact before the fix: 1 distinct dead token, 1 user, retried 559 times in 60 days (latest 2026-08-01). **Deploy still required:** `supabase functions deploy notify_friend --project-ref ilxzpszgirhwxpeocygs --no-verify-jwt`, then confirm live `verify_jwt=false` and watch for the 404 retries stopping |
