...
- Updated the home “latest photo” card to show the latest 3 feed photos as separated, tappable, drifting bubbles with fullscreen preview; room state now keeps `latest_photos` (now max 10 for Pet Home, with compact cards still previewing 3) while preserving `latest_photo` for backward compatibility.
