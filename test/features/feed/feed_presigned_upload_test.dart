...
void main() {
  group('AppConfigService.fetchBoolFlag', () {
    Future<bool> resolve(Object? value, {bool defaultValue = false}) {
      final service = AppConfigService(
        configValueLoader: (_) async => value,
      );
      return service.fetchBoolFlag('feed_presigned_upload_enabled',
  ...
  group('edge function contracts', () {
    ...
    test('feed_upload_url verifies membership and scopes the key to the room',
        () {
    ...
    test('feed_validate only accepts client image_url under the room prefix',
        () {
  ...
  group('client wiring', () {
    final client =
        File('lib/features/feed/feed_upload_client.dart').readAsStringSync();
    ...
    test('presigned upload is gated and falls back to base64', () {
      ...
      expect(
        client,
        contains("static const String presignedUploadFlagKey ="),
      );
    });
