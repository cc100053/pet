...
- [ ] Read AGENTS.md, active memory-bank files, task notes, local skills,
  referenced docs, scripts, Edge Functions, and relevant migrations.
- [ ] Archive current active memory/task snapshots before compaction.
- [ ] Add only repo-grounded workflow updates to AGENTS.md.
- [ ] Compact active memory-bank and task notes into current-state summaries.
- [ ] Run line-count/diff checks plus `flutter analyze` and `flutter test`.

## Plan (2026-05-22 App Store EULA Metadata Rejection)
- [x] Read active memory-bank files and ASC metadata workflow skills.
- [x] Confirm current legal links and live Terms of Use pages return HTTP 200.
- [x] Add explicit Terms of Use (EULA) links to local ASC version descriptions.
- [x] Add a regression test so future ASC version metadata cannot omit EULA
  links.
- [x] Update release-notes workflow guardrails to require the EULA footer.
- [x] Upload and verify the fixed App Store Connect metadata.
- [x] Run `flutter analyze` and `flutter test`, then record results.

## Review (2026-05-22 App Store EULA Metadata Rejection)
- Root cause: the ASC version descriptions did not include a functional Terms
  of Use / EULA link, which App Review requires for auto-renewable
  subscriptions.
- Fixed local ASC metadata for `en-US`, `ja`, `ko`, and `zh-Hant` by appending
  a direct Apple Standard EULA footer to each version description.
- Verified live Terms pages returned HTTP 200:
  `terms_of_use.html`, `terms_of_use_ja.html`, `terms_of_use_ko.html`,
  `terms_of_use_zh_TW.html`; Apple standard EULA URL also returned HTTP 200.
- Uploaded the fixed `.asc/version-localizations/` files to ASC version
  `2.0.0` (`1fc934d6-90c8-4b94-802c-e35d0c5e0230`) and verified the remote
  JSON descriptions include the direct Apple EULA URL:
  `https://www.apple.com/legal/internet-services/itunes/dev/stdeula/`.
- Checked App Review state after upload: version remains `REJECTED` with
  `UNRESOLVED_ISSUES`; ASC doctor says the remaining blocker is the unresolved
  App Review issue itself, so the issue must be resolved/resubmitted in App
  Store Connect after the metadata fix.
- Added `test/app_store_metadata_terms_test.dart` so future release metadata
  changes fail if any version description omits `EULA` or the direct Apple
  Standard EULA URL.
- Updated `.codex/skills/release-notes-sync/SKILL.md` and `tasks/lessons.md`
  so release-note syncs preserve the EULA footer before ASC upload.
- Verification:
  `flutter test test/app_store_metadata_terms_test.dart` passed,
