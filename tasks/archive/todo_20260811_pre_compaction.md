...
- [ ] Audit `AGENTS.md`, active memory, task notes, local workflows, recent
      commits, and the complete worktree without discarding shared-session work.
- [ ] Archive exact pre-compaction snapshots and reduce active memory/task notes
      to current decisions, contracts, and follow-ups.
- [ ] Add only verified, non-duplicative workflow guidance to `AGENTS.md`.
- [ ] Inspect all diffs, run required Flutter verification, commit logical
      groups, push `main`, verify the remote SHA, and leave a clean worktree.
