...
- [ ] Archive pre-compaction snapshots and keep active memory current-state
      focused.
- [ ] Add only newly supported, non-duplicative AGENTS workflow guidance.
- [ ] Record before/after counts and inspect the documentation-only diff.
- [ ] Run `flutter analyze` and `flutter test`, then record results.
