...
- [ ] Audit `AGENTS.md`, active memory, task notes, repo-local workflows,
      recent commits, and the full worktree.
- [ ] Archive exact pre-compaction snapshots and compact active memory/task
      notes to current decisions, contracts, and follow-ups.
- [ ] Add only verified, non-duplicative workflow guidance to `AGENTS.md`.
- [ ] Inspect the complete diff and run required formatting/analyzer/tests.
- [ ] Commit all legitimate changes in logical groups, push `main`, verify the
