...
- `lib/features/home/`: Signed-in home shell.
  ...
  - Furniture edit mode now supports selecting an already placed furniture item, dragging it with one finger, and resizing it from a large bottom control bar instead of pinch gestures. Entering furniture mode clears any stale prior selection so transform affordances stay hidden until the user taps a placed item, the bottom controls only appear for the active selection, and resize operations still persist through `public.room_furniture.scale` with the shared `0.8x..2.0x` clamp plus normalized-position reclamping at room edges.
...
- `lib/features/chat/`: Chat stream with text, feed cards, and system events.
    - `lib/features/chat/chat_room_view_v2.dart`: Sole chat-room route used by Home. It keeps Supabase fetch/realtime/cache/moderation/feed-send logic in app code while owning a deterministic timeline, the custom composer shell, reply-jump behavior, reactions, reply previews, keyboard-dismiss/back-swipe interactions, and the bright/dark status-bar overlay handoff for the in-chat top bar. Chat entry now hydrates only the latest 20 cached/server messages, auto-loads 20 older messages near the top, keeps a bounded 80-message in-memory window, buffers realtime inserts while the user is browsing history, uses a non-layout-affecting top overlay for older-history loading feedback, preserves the viewport against the first visible message while prepending older pages, and uses the floating jump button as a true reset back to the newest live window. The route now renders the room background on its own fixed layer while the entire interactive chat surface translates upward with keyboard open, so both composer and timeline content rise together regardless of whether the user is at latest or browsing history. Keyboard metric changes therefore no longer restore a fixed history anchor; instead the currently viewed message naturally lifts upward with the rest of the timeline and remains visible above the composer. Explicit `Latest` still transitions through a merged live window first and only collapses back to the newest page after the bottom animation settles, eliminating the prior extent-shrink down/up jitter during refresh and delayed layout hydration, and the `Latest` pill itself now lives inside the composer's tap region so keyboard-open taps no longer dismiss the keyboard before the jump runs. The text-send path now marks `_sending` before any pre-send async reconciliation and only restores composer draft state while mounted, reducing intermittent send-time lifecycle races around route teardown and rapid repeat submits.
      - Long-press message actions now render through a Telegram-style anchored overlay route rather than a bottom sheet. The overlay uses the pressed bubble's measured rect plus long-press coordinates to place a sharp preview over the blurred room, keeps the 6 quick reactions in a frosted rail above the preview, and keeps the existing reply/copy/moderation options in a frosted card below without changing any downstream action handlers or reaction contracts.
      - Message reactions now use two separate surfaces: long-press still opens the legacy quick-action sheet with emoji shortcuts plus `Reply` / `Copy` / moderation actions, while existing inline reaction chips open a separate WhatsApp-style reaction details bottom sheet focused on that emoji. The details sheet owns add/change/remove reaction from its top emoji bar and lists each reacting member with avatar/name/emoji without replacing the original long-press menu.
      - All chat reaction entry points now support arbitrary emoji selection through one shared bottom-sheet picker. The long-press action sheet keeps the 6 quick reactions but adds a `More` CTA that opens the full picker, the reaction-details sheet adds the same `More` path without leaving the details surface, and the fullscreen photo viewer now reuses that shared picker instead of a quick-only sheet. The picker intentionally exposes only category browsing, recents, and top suggestion chips; search mode is disabled.
    ...
    - `lib/features/chat/widgets/chat_reaction_details_sheet.dart`: New WhatsApp-style reaction details bottom sheet used by the active chat route. It renders the emoji filter bar, participant list, and in-sheet message actions while allowing inline reaction mutation from the same surface.
