...
- `lib/features/home/`: Signed-in home shell.
  ...
  - `lib/features/home/widgets/home_game_status_bar.dart`: Pet Home status bar now supports a floating room-decor guidance title anchored above the inventory/backpack chip plus a looping sweep highlight around that action, allowing Home to teach users where to tap after returning from a Shop furniture/background purchase without shifting the HUD layout.
...
- `lib/features/chat/`: Chat stream with text, feed cards, and system events.
    - `lib/features/chat/chat_room_view_v2.dart`: Sole chat-room route used by Home. It keeps Supabase fetch/realtime/cache/moderation/feed-send logic in app code while owning a deterministic timeline, the custom composer shell, reply-jump behavior, reactions, reply previews, keyboard-dismiss/back-swipe interactions, and the bright/dark status-bar overlay handoff for the in-chat top bar. Chat entry now hydrates only the latest 20 cached/server messages, auto-loads 20 older messages near the top, keeps a bounded 80-message in-memory window, buffers realtime inserts while the user is browsing history, uses a non-layout-affecting top overlay for older-history loading feedback, preserves the viewport against the first visible message while prepending older pages, and uses the floating jump button as a true reset back to the newest live window. The route now renders the room background on its own fixed layer while the entire interactive chat surface translates upward with keyboard open, so both composer and timeline content rise together regardless of whether the user is at latest or browsing history. Keyboard metric changes therefore no longer restore a fixed history anchor; instead the currently viewed message naturally lifts upward with the rest of the timeline and remains visible above the composer. Explicit `Latest` still transitions through a merged live window first and only collapses back to the newest page after the bottom animation settles, eliminating the prior extent-shrink down/up jitter during refresh and delayed layout hydration, and the `Latest` pill itself now lives inside the composer's tap region so keyboard-open taps no longer dismiss the keyboard before the jump runs. The text-send path now marks `_sending` before any pre-send async reconciliation and only restores composer draft state while mounted, reducing intermittent send-time lifecycle races around route teardown and rapid repeat submits.
    ...
    - `lib/features/chat/widgets/deterministic_chat_list.dart`: Shared deterministic timeline wrapper for the active chat route. It preserves manual keyboard-dismiss behavior, forwards long-press hooks, owns the restored localized day-separator rendering for the message timeline, no longer injects load-more UI that changes timeline height during older-history fetches, relies on the route's safe-area + measured-composer padding while the scaffold itself resizes for keyboard visibility changes, and explicitly applies Flutter's `RangeMaintainingScrollPhysics` on top of the platform scroll physics so viewport-dimension changes behave more predictably during keyboard/late-layout transitions.
