...
- `lib/features/feed/`: Camera capture, ML Kit labeling, and feed upload flow.
  ...
  - `lib/features/chat/`: Chat stream with text, feed cards, and system events.
    - `lib/features/chat/chat_message_list.dart`: Extracted stateful chat message list (pagination/realtime/cache/moderation UI state); legacy timeline scroll now uses manual keyboard-dismiss behavior so dragging messages does not collapse the composer.
    - Chat now supports modern reply/reaction interactions in the active V2 route: all non-system messages expose a long-press action sheet with quick emoji reactions plus ownership-aware `Reply` / `Copy` / moderation actions, replies render as integrated sections inside the existing composer/bubble/card surfaces instead of detached cards, grouped reaction chips render under bubbles/cards with one-reaction-per-user toggle behavior, dark-theme sent text/feed surfaces use a lighter muted teal, and the route adds a left-half right-swipe back gesture that excludes the composer region.
    - `lib/features/chat/chat_room_view_v2.dart`: `flutter_chat_ui`-based route that keeps Supabase/realtime/cache logic in-app while the package owns list/bubble rendering; the active route now uses a custom Telegram-style composer shell wired back into package callbacks/spacing, keeps timeline drags from auto-dismissing the keyboard, adds Telegram-like keyboard dismissal where a downward drag can start in the message list, enter the composer zone, and only collapse the keyboard after continuing farther down past an entry threshold, clamps keyboard collapse spacing against the safe-area inset to avoid end-of-dismiss rebound, and only remeasures composer height when composer content changes so the top composer/reply bar stays stable during keyboard animation.
