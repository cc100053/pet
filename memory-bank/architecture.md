...
- `lib/shared/ui/cached_network_image_view.dart`: Shared feed/gallery remote-image wrapper now swallows failed aspect-ratio probes and inner `Image` decode errors, leaves the outer `CachedNetworkImage` fallback UI in control when R2 assets return `404`, and derives memory/disk decode bounds from the rendered viewport so chat/gallery thumbnails do not decode original-resolution assets into memory on iOS.
- `lib/shared/ui/full_screen_photo_viewer.dart`: Fullscreen photo viewing still supports zoom/pan/save/reply/reaction flows, but now bounds both local-file and remote-image decode sizes to the current viewport pixel dimensions before handing them to `PhotoView`, preventing fullscreen entry from decoding original-resolution feed images into memory.
