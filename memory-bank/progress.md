...
- Fixed iOS push avatar rendering path: Notification Service Extension now converts incoming pushes to communication-style notifications with `INSendMessageIntent` (pet avatar as sender image), and app/extension entitlements now include `com.apple.developer.usernotifications.communication` so the system can render sender icon treatment instead of app-icon-only fallback.
