...
- Fixed the chat upward-scroll/unmount crash in `ChatRoomViewV2`: `_ReplySwipeWrapperState` no longer lazily creates its `AnimationController` from `dispose()`, so rows that were never swiped do not try to resolve `TickerMode` from a deactivated context while unmounting; the swipe-reset animation listener is now also explicitly cleaned up.
...
- Polished the active `ChatRoomViewV2` composer toward a Telegram-like chat bar: replaced the package stock composer shell with a custom rounded floating surface wired to `flutter_chat_ui` composer callbacks/height notifier, tightened the multiline input into a cleaner inner pill, added animated disabled/active send-button states, and slimmed/tinted the reply preview bar so the active chat route feels more like a messaging app while preserving package list/bubble rendering.
- Enabled the `flutter_chat_ui` migration route from Home after verification: introduced `flutter_chat_ui` / `flutter_chat_core`, a `PetChatMessageAdapter`, and `ChatRoomViewV2`, and flipped `HomeView` to open the V2 route. Supabase fetch/realtime/cache/moderation/feed-send logic still lives in app code, while the package now owns the list/composer shell for the active chat entry path. The active V2 pass also restored the glass top bar, room-members sheet entry, blocked-users management entry, and added adapter regression tests. A key constraint remains: package models support `replyToMessageId`, but `flutter_chat_ui` 2.11.1 does not appear to render quoted replies out of the box, so the migration still needs a thin reply-preview wrapper even with package data structures.
- Fixed the chat reply fetch regression after rollout: removed the main `messages` load query’s dependency on a PostgREST self-referential `messages -> messages` join and now fetch only `reply_to_message_id` on the critical path, while quoted reply previews continue loading through the existing best-effort secondary lookup so chat history still opens even if relation schema cache lags.
- Implemented modern chat reply UX end-to-end: added nullable `messages.reply_to_message_id` support via migration `20260308110504_add_message_reply_support.sql`, updated chat fetch/cache models for reply previews, added long-press `Reply`/`Copy` actions plus left-swipe-to-reply on other users' messages, rendered inline quoted previews inside reply bubbles, and added a dismissible composer reply preview bar that sends `reply_to_message_id` with outgoing text messages.
...
- Rebuilt the fullscreen Photo Viewer around the `photo_view` package instead of a custom `InteractiveViewer` stack: `FullScreenPhotoViewer` now uses `PhotoViewGallery` for mature centering/zoom/pan behavior, keeps sender/time/comment/download chrome as floating overlays on top, and simplifies bottom chrome so it no longer visually drags the image off-center.
- Added a lightweight first-login profile setup step ahead of create-pet onboarding: fresh users now see a modal profile card on Room Selection where nickname must be customized before continuing, while avatar upload is available but optional and updates local profile state immediately after upload.
- Fixed stale latest-photo refresh behavior across room entry and room selection: entering a room now shows a lightweight gallery loading indicator while `_refreshLatestFeed` fetches fresh photos, the refresh path now syncs latest-photo metadata back into `_myRooms`/bootstrap cache so Room Selection cards update immediately after a room refresh, app resume refreshes the active room gallery, and Room Selection refresh now shows an explicit loading indicator while running its per-room pet tick refresh concurrently for faster perceived updates.
- Updated the chat composer input action to match native iPhone multiline behavior: `ChatRoomView` no longer requests the keyboard-level send action, so the iOS keyboard bottom-right key stays as Enter/return while message sending remains on the dedicated composer send button.
...
- Polished the onboarding coach card presentation to better match Room Selection: the card now uses a softer layered gradient, stronger information hierarchy, a subtle entrance animation, and a small pointer that visually anchors the guidance to the highlighted `Create New Room` CTA.
...
- Implemented backward-compatible multi-invite-code architecture: added migration `20260305231000_add_multi_invite_codes_and_member_generation.sql` (new `room_invite_codes` table + backfill + member-capable `create_room_invite_code` / `list_room_invite_codes` / `revoke_room_invite_code` RPCs), updated `join_room_by_code` to validate new-code table first with legacy fallback, and updated `create_room`/`regenerate_invite_code` to sync primary legacy `rooms.invite_code`; Home invite flow now uses the new creation RPC for all active room members and shows up to 3 active codes in the invite dialog.
- Investigated and fixed room invite-code failure path: Home invite CTA was always calling owner-only `regenerate_invite_code`, so member roles consistently failed with `not_owner`; updated invite flow to branch by room role (owners regenerate, members open current room code without RPC) and added `not_owner` -> permission-denied mapping in `userFacingError` to avoid generic retry messaging.
- Implemented iOS review fallback-to-store flow: `ReviewPromptService` now attempts in-app `requestReview()` at feed milestones and falls back to `openStoreListing(appStoreId: 6757725650)` when unavailable/failing; added testable review/settings abstractions, analytics events (`review_prompt_shown`, `review_store_opened`, `review_prompt_failed`, `review_store_open_failed`), and new unit tests in `test/review_prompt_service_test.dart` covering success/fallback/failure paths.
...
- Simplified chat keyboard-dismiss behavior per follow-up preference: switched `ChatMessageList` list views to built-in `ScrollViewKeyboardDismissBehavior.onDrag` and removed `ChatRoomView`’s custom 24px drag-dismiss strip, keeping tap-outside dismiss on the message-list layer for explicit close.
- Fixed chat composer tap focus regression where keyboard opened then immediately collapsed: in `ChatRoomView`, added stable `Stack` child keys (`chatMessageList`, `chatComposer`, keyboard underlay/dismiss strip) so keyboard-visibility tree updates no longer remount the `TextField`; kept dismiss strip always mounted via `IgnorePointer`, restored message-list-layer tap-outside dismissal, and kept pull-down dismiss on downward drag (`dy > 0`) so both dismiss affordances work without reintroducing focus collapse.
- Removed chat message-list tap-to-unfocus wrapper after repeated input-focus regression reports; keyboard dismissal now avoids full-list tap recognizer conflicts and relies on dedicated dismiss gestures.
- Hardened chat keyboard dismiss interaction after follow-up regression report: removed global pointer listener for dismiss tracking and switched to a dedicated 24px drag strip above the composer for pull-down keyboard dismissal, preventing input-field tap focus from being canceled by dismiss logic.
- Fixed chat keyboard auto-collapse regression after input tap: tightened keyboard-dismiss gesture start conditions so dismissal can begin only from a slim band above the composer (not inside the composer/input area), preventing tap-to-focus from being misinterpreted as a pull-down dismiss gesture.
- Refined chat keyboard interaction to closer Telegram behavior: removed list overscroll-triggered unfocus path, kept message list keyboard dismiss mode `manual` so scrolling messages does not collapse keyboard, added lower-zone downward drag detection to dismiss keyboard, switched chat scaffold to `resizeToAvoidBottomInset: false` with manual keyboard inset padding for list/composer layout, and replaced full-scaffold iOS keyboard tinting with a keyboard-area-only underlay to eliminate rounded-corner gray artifacts.
...
- Added a Home Debug Tools toggle to force onboarding visibility on every app open (admin-only effect): persisted `debugAlwaysShowOnboarding` in app settings, wired drawer switch control, and applied onboarding Step 1 override/reload behavior in `home_onboarding_flow`.
- Implemented onboarding v1 runtime wiring: added persisted basic-onboarding state in `AppSettingsRepository`, integrated Home-level onboarding loader/progression, added Room Selection create-CTA highlight hook, and shipped a game-style Step 1 coach card with skip action (`lib/features/home/flows/home_onboarding_flow.dart`).
- Drafted a lightweight game-style onboarding tutorial spec for first-login core actions (create pet, open room, invite friend, feed), including implementation-ready anchor IDs and Flutter state schema (`docs/onboarding_basic_tutorial_spec.md`).
- Fixed iPad/tablet scale-tier mismatch by introducing shared adaptive layout helpers (`lib/shared/ui/adaptive_layout.dart`) and routing both global `appUiScale` + Home breakpoint selection through the same effective-width logic (tablet widths now normalize to expanded-tier width `540` for scale selection after iPad Air validation); also constrained Room Selection content width on tablet (`maxWidth 560` when width >= `700`) to prevent oversized cards and mispositioned elements during App Review iPad checks.
- Added iPad compatibility-mode hardening: detect iOS tablet display from physical display metrics and force expanded-tier scale/breakpoint selection even when the app viewport width is narrow (e.g., iPhone-compat mode on iPad), preventing accidental compact/smallest-scale selection.
- Fixed scale-path inconsistency between room selection and pet home cards: Room Selection card internals now consume `homeUiScale` for key spacings/sizes/badges/CTA so both screens respond to the same scale factor; added debug prints (`[UI_SCALE]`, `[ROOM_SELECTION_LAYOUT]`) to verify active scale and breakpoint at runtime.
- Tuned Pet Home gallery size by tier only: center feed photo cards now apply an extra visual scale factor in `PetPhotoGallery` (`compact 0.90`, `regular 0.94`, `expanded 1.00`) so compact/regular devices render smaller gallery cards without changing expanded layout.
- Finalized pet-home vertical allocation with Option 2 (ratio + clamp): `HomeMainContent` now splits the center body into gallery/pet movement regions using a 50/50 baseline with tier-aware min/max gallery height clamps, so wide iPad layouts no longer collapse pet movement area even when the same scale tier is used.
- Increased iPhone gallery height slightly while keeping iPad behavior stable: `HomeMainContent` now uses tiered split ratios (`compact 0.64`, `regular 0.62`, `expanded 0.60`) so compact/regular phones reserve more gallery area without changing expanded-tier tablet composition.
...
- Fixed post-delete navigation on Profile: after successful account deletion cleanup sign-out, `ProfileView` now returns to the root navigator when the session becomes null so users land on the login screen instead of staying on the Profile route.
