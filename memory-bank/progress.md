...
- Fixed compact/regular pet-home movement-space bottleneck by reducing gallery layout occupancy (not just visual size): `PetPhotoGallery` aspect ratios now reserve less vertical space on compact/regular (`1.44` / `1.32`), and `HomeMainContent` now uses tier-aware smaller vertical gaps/padding for compact/regular so the pet movement field gets more actual area.
