...
## Done
- Added a default-off `flutter_chat_ui` migration spike: introduced `flutter_chat_ui` / `flutter_chat_core`, a `PetChatMessageAdapter`, and `ChatRoomViewV2` that keeps Supabase fetch/realtime/cache/moderation logic in app code while proving package-based list/composer integration for text, custom feed cards, system messages, long-press reply actions, Telegram-style swipe reply, and quote jump-to-source highlight. The spike also confirmed an important constraint: package models support `replyToMessageId`, but `flutter_chat_ui` 2.11.1 does not appear to render quoted replies out of the box, so production migration will still need a thin reply-preview wrapper even when using package data structures.
