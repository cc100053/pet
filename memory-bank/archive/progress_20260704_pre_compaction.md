...
- Feed upload has a durable queue, presigned direct-to-R2 support behind
  `app_config.feed_presigned_upload_enabled`, base64 fallback, and
  authoritative satiety reconciliation from `feed_validate`.
- `feed_validate` reward/message writes stay on the response path; partner push
  dispatch stays in `EdgeRuntime.waitUntil`; old response field types remain
  stable.
...
- ASC metadata for auto-renewable subscriptions must retain the direct Apple
  Standard EULA footer in every `.asc/version-localizations/*.strings`
