...
- Room entry warms across launches from cached per-room `pet_state`/`pet_id`;
  cold entry seeds the known main `pet_id`, and picker-only decor loads
  post-frame to avoid entry-critical read contention.
