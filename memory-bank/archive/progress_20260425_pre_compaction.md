...
- Pet dress-up MVP is wired end-to-end for app `1.3.0`: the live
  ...
  The Godot Socket Authoring add-on now also has selector-driven preview setup:
  it scans pet/action scenes, opens the selected scene, auto-loads matching
  socket JSON, scans Flutter equipment PNGs, and includes a clear equipment
  option for previewing the pet without an overlay.
